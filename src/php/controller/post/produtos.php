<?php
    ...
    if ($response_data !== null && json_last_error() === JSON_ERROR_NONE) {
        if (isset($response_data['status']) && $response_data['status'] === 202) {
            salvarLog($arquivoLogSuccess, "Sucesso (Produtos): " . json_encode($response_data));
            echo json_encode([
                "success" => true,
            ...
            salvarLog($arquivoLogError, "Erro no retorno da API (Produtos): " . json_encode($response_data));
            echo json_encode([
                "success" => false,
        ...
        salvarLog($arquivoLogError, "Erro ao decodificar JSON de resposta (Produtos): $erroJson");
